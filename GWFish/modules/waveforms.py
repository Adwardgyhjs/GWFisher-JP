...
def hphc_amplitudes(waveform, parameters, frequencyvector,once_flag,flag='ins',plot=None):
    parameters = parameters.copy()

    if waveform == 'gwfish_TaylorF2':
        hphc = TaylorF2(parameters, frequencyvector, plot=plot)
    elif waveform == 'gwfish_IMRPhenomD':
        hphc = IMRPhenomD(parameters, frequencyvector,once_flag,flag=flag,plot=plot)
...
def JP_isco(chi,epsilon):
    return kerr_isco(chi)*(1.-epsilon*(1./27.+(37.*chi)/(324.*6.**(1/2))+(1229.*chi**2)/23328.))

def JP_j_chi(x,epsilon):
    return 2./(3*JP_isco(x,epsilon))**(0.5)*(3*(JP_isco(x,epsilon))**(0.5) -2*x)
def JP_epsilon_chi(x,epsilon):
    return (1.-2./JP_isco(x,epsilon)+x/(JP_isco(x,epsilon))**(3/2))/(1-3./JP_isco(x,epsilon)+2*x/(JP_isco(x,epsilon))**(3/2))**(0.5)
def epsilon_chi(x):
...
def final_bh(M1, M2, chi1, chi2):
    ...
    return chi_f, m_f
def Mt_of_Mf(eta,chi_f,Mf,epsilon):
    Eisco=JP_epsilon_chi(chi_f,epsilon)
    Mt=Mf/(1+eta*(Eisco-1))
    return Mt
...
#JP metric
def Lambda(M_t,mu):
    return np.sqrt(np.abs(M_t*(M_t-4.*mu)))

def kappa(mu,chi_a,M_t,delta_m,Lambda,chi_s):
    return np.sqrt(8.*np.sqrt(3)*mu*chi_a*(M_t*delta_m+Lambda)+8.*np.sqrt(3)*mu*chi_s*(delta_m*Lambda+M_t)+\
                   3.*(40.*mu**2+9.*M_t**2+12.*np.sqrt(2)*mu*M_t))

def delta_Mf(epsilon,delta_chi_f,chi_f_kerr,mu):
    return -(epsilon*mu*(864.*delta_chi_f*(5.*np.sqrt(2)*chi_f_kerr+3*np.sqrt(3))+\
                                  545.*np.sqrt(2)*chi_f_kerr**2+324.*np.sqrt(3)*chi_f_kerr+216.*np.sqrt(2)))/139968

def delta_chi_f(epsilon,kappa,mu,Lambda,M_t,chi_s,chi_a,delta_m):
    return -epsilon*(1./(384.*np.sqrt(3)*kappa*mu))*(420.*mu*M_t*chi_a*delta_m+420.*mu*chi_a*Lambda-\
                                                   152.*np.sqrt(2)*kappa*mu+2416.*np.sqrt(3)*mu**2+420.*mu*\
                                                   delta_m*Lambda*chi_s+945.*np.sqrt(3)*M_t**2-315.*kappa*M_t+\
                                                   1086.*np.sqrt(6)*mu*M_t+420.*mu*M_t*chi_s)
def IMRPhenomD(parameters, frequencyvector,once_flag,flag,plot=None):
    phic = parameters['phase']
    tc = parameters['geocent_time']
    z = parameters['redshift']
    r = parameters['luminosity_distance'] * cst.Mpc
    iota = parameters['theta_jn']
    epsilon_index=int(flag[-1])
    epsilon_current_thread=parameters['epsilon']
    ####################从生成的参数里读取参数值#################################
    C = 0.57721566  # Euler constant
    eta=parameters['eta']
    delta_mass=parameters['delta_mass']
    mu=parameters['mu']
    chi_eff=parameters['chi_eff']
    chi_PN=parameters['chi_PN']
    chi_s=parameters['chi_s']

    m_f=[parameters['Mf']*(1+z)*cst.Msol]
    chi_f=[parameters['af']]
    Mf_Msol = parameters['Mf']
    M = Mt_of_Mf(eta, chi_f[0], Mf_Msol, epsilon_current_thread) * (1 + z) * cst.Msol
    Lisco=JP_j_chi(chi_f[0],epsilon_current_thread)*(M/((1 + z) * cst.Msol))
    ff = frequencyvector * cst.G * M / cst.c ** 3
    ones = np.ones((len(ff), 1))
    Mc=cst.G*M*eta**(3/5)/cst.c**3
    #Mc = M * eta ** (3 / 5) /cst.Msol
    eta2=eta*eta
    eta3=eta2*eta

    if flag=='ins_Modify_{}'.format(epsilon_index):
        chi_a=(chi_f[0]-(1-2*eta)*(M/m_f[0])*chi_s-(1/Mf_Msol)*eta*Lisco)/(np.sqrt(1-4*eta)*(M/m_f[0]))
        #print("{} chi_a {}".format(flag,chi_a))
        #反对称自旋参数是非物理所允许的即需要-1<chi_a<1，极化张量返回[None,None]
        #if chi_a > 1 or chi_a < -1:
            #print("{}非物理所允许的反对称自旋:{}".format(flag,chi_a))
            #return np.hstack(([None],[None]))
    elif flag=='MR_Modify_{}'.format(epsilon_index):
        chi_a=(chi_f[0]-(1-2*eta)*chi_s-(1/Mf_Msol)*eta*Lisco)/np.sqrt(1-4*eta)
        #print("{} chi_a {}".format(flag, chi_a))
        #if chi_a > 1 or chi_a < -1:
            #print("{}非物理所允许的反对称自旋:{}".format(flag, chi_a))
            #return np.hstack(([None],[None]))

    Fisco=(aux.fisco(M)/(cst.c**3/(M*cst.G)))*ones
    #print("Fisco wave",Fisco)
    #########################################################################################################
    #########################################################################################################
    #########################################################################################################
    ...
    beta_JP=(75.*epsilon_current_thread)/(64.*eta)
    #print("{}beta_JP==={}".format(threading.current_thread().name,beta_JP))
    psi_TF2 = 2.*np.pi*ff*cst.c**3/(cst.G*M)*tc - phic*ones - np.pi/4.*ones + 3./(128.*eta)*((np.pi*ff)**(-5./3.) +\
            ...
            phi_5 + phi_6*(np.pi*ff)**(1./3.) + phi_7*(np.pi*ff)**(2./3.))+beta_JP*(np.pi*ff)**(-1./3.)
    ...
    """
    if flag=='ins' or flag=='MR':
        chi_f, m_f = final_bh(M1, M2, chi_1, chi_2)

        chi_f_temp=chi_f[0]
        m_f_temp=m_f[0]
        chi_M_MR=np.array([[m_f_temp/((1+z)*cst.Msol),chi_f_temp]])

        with open("chi_M_MR.txt", "ab") as f:
            np.savetxt(f, chi_M_MR,fmt='%f',delimiter=' ')
    #np.savetxt('chi_m_GRVal.txt',chi_M_GR)
    """
    Mff=parameters['Mf']
    #JP度规对似正规模频率的修正
    omega_RD_JP=epsilon_current_thread*(1./(81.*np.sqrt(3.)*Mff)+(10./(729.*Mff))*chi_f[0]+(47./(1458.*np.sqrt(3.)*Mff))*chi_f[0]**2)#*(cst.c**3/cst.G)
    #print("omega RD ",omega_RD_JP)
    omega_damp_JP=epsilon_current_thread*((1./(486*Mff))*chi_f[0]+(16./(2187.*np.sqrt(3.)*Mff))*chi_f[0]**2)#*(cst.c**3/cst.G)
    #print("omega damp",omega_damp_JP)
    data_ff = np.loadtxt('n1l2m2.dat', unpack = True)
    M_omega = interp1d(data_ff[0, :], data_ff[1, :])
    tau_omega = interp1d(data_ff[0, :], data_ff[2, :])
    #print("M_omega",M_omega(chi_f))

    ff_RD = ((M_omega(chi_f)+omega_RD_JP)/(2*np.pi)*M/m_f)[0]
    ff_damp = (-(tau_omega(chi_f)-omega_damp_JP)/(2*np.pi)*M/m_f)[0]
    ...
    alpha_JP=(3.*epsilon_current_thread)/2.

    amp_PN = (a_0 + a_2*(np.pi*ff)**(2./3.) + a_3*(np.pi*ff) + a_4*(np.pi*ff)**(4./3.) +\
            a_5*(np.pi*ff)**(5./3.) + a_6*(np.pi*ff)**2.+alpha_JP*(np.pi*ff)**(4./3.))
    #print("{}amp_PN==={}".format(threading.current_thread().name, amp_PN))
    ...
    #print("{}test f3_amp====={}".format(threading.current_thread().name, f3_amp))
    ...
    #print("A0",A0)
    #lnAGR=parameters['lnAGR']
    DL= parameters['luminosity_distance']
    lnAGR=np.log(Mc ** (5. / 6.) / ((30 ** (1 / 2)) * (np.pi ** (2. / 3.)) *  DL*cst.Mpc))
    A0=np.exp(lnAGR+np.log(((25./4.)**(0.5)*cst.c*frequencyvector**(-7./6.))))
    #print("A0 LNAGR",A0)
    ...
    #以Fisco为分界频率区分旋近与并合
    theta_minus_ins=0.5*(1*ones-step_function(ff,Fisco))
    theta_plus_MR=0.5*(1*ones+step_function(ff,Fisco))

    #旋近相位
    if flag=='ins_Modify_{}'.format(epsilon_index):
        Psi_I=psi_tot*theta_minus_ins
        #psi_tot = psi_ins +psi_int#psi_int+psi_MR #psi_ins + psi_int + psi_MR
        phase_I = np.exp(1.j * Psi_I)

    #并合相位
    elif flag=='MR_Modify_{}'.format(epsilon_index):
        Psi_MR=psi_tot*theta_plus_MR
        phase_MR = np.exp(1.j * Psi_MR)
        #psi_tot = psi_MR

    #旋近振幅
    if flag=='ins_Modify_{}'.format(epsilon_index):
        Amp_I=amp_tot*theta_minus_ins
        #amp_tot = amp_ins + amp_int #amp_int+amp_MR #amp_ins + amp_int + amp_MR

    #并合振幅
    elif flag=='MR_Modify_{}'.format(epsilon_index):
        #print("{}amp_MR==={}".format(threading.current_thread().name, amp_MR))
        #amp_tot = amp_MR
        Amp_MR=amp_tot*theta_plus_MR

    #旋近阶段叉加模式的hstack
    #hstack:水平压栈
    if flag == 'ins_Modify_{}'.format(epsilon_index):
        hp = Amp_I*0.5*(1 + np.cos(iota)**2.)
        hc = Amp_I*np.cos(iota)
        polarizations = np.hstack((hp * phase_I, hc * 1.j * phase_I))

    #并合阶段叉加模式的hstack
    elif flag == 'MR_Modify_{}'.format(epsilon_index):
        hp = Amp_MR*0.5*(1 + np.cos(iota)**2.)
        hc = Amp_MR*np.cos(iota)
        polarizations = np.hstack((hp * phase_MR, hc * 1.j * phase_MR))

    np.savetxt("Fisco_{}.csv".format(flag),[Fisco[0]*cst.c**3/(M*cst.G)])
    if plot is not None:
        plt.figure()
        y_height = plot[3]/10
